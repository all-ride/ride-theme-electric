...
{if $table->hasRows() || $table->hasSearch() || $tableActions}
    ...
    <form id="{$tableForm->getId()}" action="{if isset($tableAction)}{$tableAction}{else}{$table->getFormUrl()}{/if}" method="POST" class="table spacer" role="form" data-confirm-messages="{htmlspecialchars($tableMessages)}">
                ...
                <div class="grid--bp-med__4 order text--right form__item">
                    {if $table->hasOrderMethods()}
                        {block name="table.order"}
                            ...
                            <a href="{str_replace("%direction%", $direction, $table->getOrderDirectionUrl())}">
                                <i class="{$iconClass}"></i>
                            </a>
                        {/block}
                    {/if}
