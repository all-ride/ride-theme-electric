...
{block name="content_body" append}
    <div class="breadcrumb">
        <span itemscope itemtype="http://data-vocabulary.org/Breadcrumb" class="breadcrumb__item{if count($breadcrumbs) == 0} breadcrumb__item--active{/if}">
          ...
          </a>{if count($breadcrumbs) != 0}  &rsaquo;{/if}
        </span>
    ...
    <form action="{url id="assets.asset.add" parameters=["locale" => $locale]}?folder={$folder->id}&embed={$embed}"
        ...
        data-placeholder="{translate key="label.upload.dnd.placeholder"}"
    ...
    <form id="{$form->getId()}" class="form form-filter{*  form--selectize *}" action="{$app.url.request}" method="POST" role="form">
        <div class="actions clearfix spacer">
            <div class="btn-group">
                <a href="{url id="assets.asset.add" parameters=["locale" => $locale]}?folder={$folder->id}&embed={$embed}&referer={urlencode($app.url.request)}" class="btn btn--default btn--small">
                   {translate key="button.add.asset"}
                </a>
                <a href="{url id="assets.folder.add" parameters=["locale" => $locale]}?folder={$folder->id}&embed={$embed}&referer={urlencode($app.url.request)}" class="btn btn--default btn--small">
...
{block name="scripts" append}
    ...
    {if !isset($app.javascripts[$script]) && $embed}
        <script src="{$app.url.base}/electric/js/assets.js"></script>
    {/if}
