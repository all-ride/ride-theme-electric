        ...
        <tbody class="asset-items--folders">
            {foreach $folders as $item}
                ...
                <tr class="order-item" data-type="{$type}" data-id="{$item->getId()}">
                    ...
                    <td>
                        <a href="{url id="assets.folder.overview" parameters=["locale" => $locale, "folder" => $item->id]}{str_replace("&page=`$page`", "", $urlSuffix)}">
                            <img src="{$app.url.base}/electric/img/folder.svg" width="50" height="50" class="data img-responsive" />
                        </a>
                        <a href="{url id="assets.folder.edit" parameters=["locale" => $locale, "folder" => $item->getId()]}?embed={$embed}&referer={urlencode($app.url.request)}">
                            {$item->getName()}
                        </a>
            ...
            {foreach $assets as $item}
                ...
                <tr class="order-item" data-type="{$type}" data-id="{$item->getId()}">
                    ...
                    <td>
                        ...
                        {/if}
                        <a href="{url id="assets.asset.edit" parameters=["locale" => $locale, "asset" => $item->getId()]}?embed={$embed}&referer={urlencode($app.url.request)}">
                            {$item->getName()}
                        <a>
                        <div class="text-muted">
                            {translate key="label.type.`$type`"}
