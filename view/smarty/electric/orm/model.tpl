...
{block name="content_body" append}
    <dl>
        ...
        <dd>{if $hasApi}<a href="{url id="api.class"}/{str_replace("\\", "/", $modelClass)}">{$modelClass}</a>{else}{$modelClass}{/if}</dd>
        <dt>{translate key="label.class.entry"}</dt>
        <dd>{if $hasApi}<a href="{url id="api.class"}/{str_replace("\\", "/", $entryClass)}">{$entryClass}</a>{else}{$entryClass}{/if}</dd>
        <dt>{translate key="label.class.proxy"}</dt>
        <dd>{if $hasApi}<a href="{url id="api.class"}/{str_replace("\\", "/", $proxyClass)}">{$proxyClass}</a>{else}{$proxyClass}{/if}</dd>
